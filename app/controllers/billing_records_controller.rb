class BillingRecordsController < ApplicationController
  ...
  def create
    ...
    new_billing_record = BillingRecord.new(
      user_id: current_user.id, 
      type: billing_record_params[:type], 
      name: billing_record_params[:name], 
      amount: billing_record_params[:amount] * 100
    )
    billing_room.update(
      billing_records: billing_room.billing_records.push(new_billing_record.to_h)
    )
  end
  ...
  def billing_record_params
    params.require(:billing_record).permit(:name, :amount, :type)
  end
end
